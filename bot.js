    ...
    const response = await fetch(`${OPENAI_BASE_URL}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `Язык: русский. Текст от пользователя может быть на русском — обрабатывай его как есть и формируй поисковые запросы на том же языке (если текст на русском, запросы на русском).
Создай 2-3 поисковых запроса для Google чтобы найти первоисточник этой информации.
Запросы должны содержать ключевые факты, имена, даты из текста.
Верни только JSON массив строк: ["запрос 1", "запрос 2"]`
          },
    ...
    const response = await fetch(`${OPENAI_BASE_URL}/chat/completions`, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `Язык: русский. Исходный текст и все ответы (reason, explanation) — только на русском.
Ты эксперт по проверке информации. Проанализируй текст и источники.
Верни JSON:
{
  "sources": [{"index": 1, "relevance": 85, "isLikelyOriginal": true, "reason": "причина на русском"}],
  "confidence": 75,
  "explanation": "вывод на русском"
}`
          },
          {
            role: 'user',
