...
- [x] Настроить проект на Vercel
- [x] Добавить переменные окружения в Vercel
- [x] Задеплоить проект
- [x] Зарегистрировать webhook в Telegram:
  ```powershell
...
- [x] Обработка ошибок и edge-cases (устойчивый парсинг JSON, DuckDuckGo 403)
...
| `OPENAI_API_KEY` или `OPENROUTER_API_KEY` | Ключ OpenAI или OpenRouter | Да (один из двух) |
| `OPENAI_MODEL` или `OPENROUTER_MODEL` | Модель (напр. nvidia/..., openai/gpt-oss-120b) | Нет (fallback: gpt-4o-mini) |
| `OPENAI_BASE_URL` | URL API (для OpenRouter: https://openrouter.ai/api/v1) | Нет (подставляется при OPENROUTER_API_KEY) |
| `GOOGLE_API_KEY` | Ключ Google Custom Search API | Нет* |
| `GOOGLE_SEARCH_ENGINE_ID` | ID поискового движка Google | Нет* |

*Без Google ключей используется DuckDuckGo (на Vercel часто 403 — рекомендуется настроить Google)
